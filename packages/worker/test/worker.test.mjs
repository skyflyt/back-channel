...
test('uncertain execution cleanup survives restart until confirmed recovery, while its result is delivered', async t => {
    const s = setup(t);
    const first = await s.a.send({targetAgentId: 'b', profile: 'approved', objective: 'first'});
    const second = await s.a.send({targetAgentId: 'b', profile: 'approved', objective: 'second'});
    s.b.runner = async () => ({status: 'interrupted', text: 'Cleanup is uncertain', requiresRecovery: true});
    await assert.rejects(s.b.cycle(), {code: 'RECOVERY_REQUIRED'});
    assert.equal(s.relay.tasks.get(first).status, 'interrupted');
    assert.equal(s.b.journal.tasks[first].state, 'delivered');
    assert.equal(s.b.store.read('journal').recoveryRequired.taskId, first);
    let calls = 0;
    const restarted = new Worker(s.b.store, {client: s.relay.client('b'), runner: async () => {calls++; return {status: 'completed', text: 'done'};}});
    await restarted.recover();
    await assert.rejects(restarted.cycle(), {code: 'RECOVERY_REQUIRED'});
    assert.equal(calls, 0);
    assert.equal(s.relay.tasks.get(second).status, 'queued');
    await restarted.recover({confirmStopped: true});
    await restarted.cycle();
    assert.equal(calls, 1);
    assert.equal(s.relay.tasks.get(second).status, 'completed');
    assert.equal(s.b.store.read('journal').recoveryRequired, undefined);
    assert.equal(s.b.store.read('journal').lastConfirmedRecovery.previous.taskId, first);
});
test('uncertain continuation cleanup persists its recovery block and the CLI exits with recovery instructions', async t => {
    const s = setup(t);
    const id = await s.a.send({targetAgentId: 'b', profile: 'approved', objective: 'continue', continuationProfile: 'approved'});
    s.b.runner = async () => ({status: 'completed', text: 'done'});
    s.a.runner = async () => ({status: 'interrupted', text: 'Cleanup is uncertain', requiresRecovery: true});
    await s.b.cycle();
    await assert.rejects(s.a.cycle(), {code: 'RECOVERY_REQUIRED'});
    assert.equal(s.a.store.read('journal').recoveryRequired.phase, 'continuation');
    assert.equal(s.a.store.read('journal').recoveryRequired.taskId, id);
    const config = s.a.store.read('config');
    config.broker = 'http://127.0.0.1:1'; config.token = 'fixture-only';
    s.a.store.write('config', config);
    const cli = path.join(import.meta.dirname, '../bin/cli.mjs');
    const blocked = spawnSync(process.execPath, [cli, '--state', s.a.store.directory, 'run'], {encoding: 'utf8', timeout: 10000, windowsHide: true});
    assert.equal(blocked.status, 1);
    assert.match(blocked.stderr, /recover --confirm-stopped/);
    assert.ok(s.a.store.read('journal').recoveryRequired);
    const unconfirmed = spawnSync(process.execPath, [cli, '--state', s.a.store.directory, 'recover'], {encoding: 'utf8', timeout: 10000, windowsHide: true});
    assert.equal(unconfirmed.status, 1);
    assert.ok(s.a.store.read('journal').recoveryRequired);
    const confirmed = spawnSync(process.execPath, [cli, '--state', s.a.store.directory, 'recover', '--confirm-stopped'], {encoding: 'utf8', timeout: 10000, windowsHide: true});
    assert.equal(confirmed.status, 0, confirmed.stderr);
    assert.equal(s.a.store.read('journal').recoveryRequired, undefined);
});
