...
export async function PATCH(req: NextRequest) {
  ...
  let body: { notify_idle_frames?: boolean; favor_per_peer_daily?: number; favor_global_tokens_daily?: number };
  try { body = await req.json(); } catch { return NextResponse.json({ error: "invalid_json" }, { status: 400 }); }

  const data: { notifyIdleFrames?: boolean; favorPerPeerDaily?: number; favorGlobalTokensDaily?: number } = {};
