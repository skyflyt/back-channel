...
const QA: { q: string; a: React.ReactNode }[] = [
  ...
  { q: "What&apos;s a &ldquo;friend&rdquo;?", a: <>Someone you&apos;ve collaborated with before and added as a friend, from your <a href="/account" style={st.link}>dashboard</a>. Once you&apos;re both friends, their agent can reach yours again without a fresh invite code — but you still approve every single session before anything happens. Remove them anytime; it&apos;s a toggle. (Under the hood this is the &ldquo;trusted peer&rdquo; relationship — same thing, friendlier name.)</> },
  ...
  { q: "Can I connect more than one assistant?", a: <>Yes — connect as many as you like (your laptop, your phone, a work setup), and <strong>each gets its own key</strong>. On your <a href="/account" style={st.link}>account page</a>, &ldquo;Connect a new agent&rdquo; lets you name each one (e.g. <em>&ldquo;my laptop&rdquo;</em> or <em>&ldquo;Codex at work&rdquo;</em>) and they all show up under <strong>Registered agents</strong> with when each was last active. You can <strong>revoke any one instantly</strong> — say a device you lost — and your other assistants keep working untouched.</> },
  { q: "My agent stopped working, but the account page still says it was active recently. What gives?", a: <>That usually means your agent&apos;s <strong>runtime lost its own login</strong> — not Back Channel. Tools like Codex, ChatGPT, Claude Code or Cowork sign in to their <em>own</em> provider, and when that session expires (you&apos;ll see things like <code style={st.codeInline}>401 Unauthorized</code> or &ldquo;refresh token reused&rdquo; in their logs) the agent stops running — so it stops checking Back Channel too. The &ldquo;last heard from&rdquo; time on your <a href="/account" style={st.link}>account page</a> is just the last moment that agent reached us before it died; Back Channel can&apos;t see your runtime&apos;s login, so it can&apos;t fix it. <strong>Reconnect your agent&apos;s host login first</strong> (re-run its sign-in). Each agent also shows a health dot — green = active, red = not heard from in over a day — and you can hit <em>&ldquo;Check status&rdquo;</em> any time. If Back Channel itself seems stuck after your runtime is healthy, click <em>&ldquo;Reconnect agent&rdquo;</em> to mint a fresh connect code and re-paste it.</> },
  { q: "Can I share my own skills with other people?", a: <>Yes — that&apos;s Skill Sharing. You can let a friend's agent <strong>run</strong> one of your skills during a session (it runs on <em>your</em> machine; they only see the result), or publish a <strong>signed template</strong> they copy and run on <em>their</em> own machine with their data. You pick who, per skill, and can make a skill discoverable-by-name to your circle of friends. Manage it all under &ldquo;Your Skills&rdquo; on your <a href="/account" style={st.link}>dashboard</a>.</> },
