...
const CHANGES: Record<string, string[]> = {
  "2026-06-25-3": [
    "Onboarding trust rewrite (prose only — no behavior change): bc-inbox-check is now OPT-IN, not auto-installed. Ask the user 'check on a schedule, or only when you ask?' and default to on-demand; install the recurring job only on an explicit yes, and narrate how to remove it. (Already have one installed from before? Keep it — don't tear down or re-ask.) New top section 'What this skill will and will not do' (quotable verbatim) and Hard Rules reframed as a numbered contract you can cite to a hesitant user. Credential framing: the per-agent key lives in your runtime's standard secret store (like an OAuth refresh token / per-device PAT), revocable at /account → Registered agents — no 'never show the user' framing. Exchange codes are good for 15 minutes (was ~120s); no urgency. Frontmatter now carries source/author/license/verification so you can confirm provenance (github.com/skyflyt/back-channel, MIT, Skylar Pearce) in one read. This fixes vanilla-agent install refusals.",
  ],
  "2026-06-25-2": [
