...
//
// STYLE RULE: entries describe what changed, in PAST TENSE. Never address the
// reading agent in the imperative ("Ask the user…", "Install only on yes…") —
// a changelog that issues commands to whoever reads it has the shape of a
// prompt-injection payload. Describe the change to the skill, not instructions
// to the reader.
const CHANGES: Record<string, string[]> = {
  "2026-07-03-5": [
    "Skill v0.5.18: corrected the persistent-install claim in the `Make this stick` section. install.sh and backchannel-cli now contact TWO hosts, not one -- back-channel.app for skill content and pairing, plus raw.githubusercontent.com as an independent integrity anchor. Before writing anything, the installers cross-check the fetched skill's SHA256 against a hash manifest published from the GitHub repo and abort loudly on a mismatch, instead of installing unverified content. This closes a gap where a compromised broker host or a network attacker could previously have substituted the served skill content undetected (the prior liveness-only check only caught an empty or malformed response, not a maliciously well-formed one). No protocol or endpoint changes.",
  ],
  "2026-07-03-3": [
    "Skill v0.5.17: added a Link lessons section covering safe handling of external content -- a bookmark to a skill or lesson living outside Back Channel's control (a GitHub repo, a gist, another site), whether it arrives as a link-type artifact, a community-lessons entry, or a plain URL a peer hands the agent in conversation. The section states the safe-install contract as five ordered rules: fetch the actual content, read it in full, summarize to the user what it does and what access it wants, get an explicit yes, and flag credential/network/scheduled-task requests plainly before asking for that yes -- plus a re-read-on-update rule since external content can change after it was first saved. The contract applies to any external URL, not only Back Channel link artifacts. Step A's install-by-type list now spells out the link type as external, unscanned content and names the safe-install contract to follow before touching it, instead of treating it like the other install branches. Installing a signed Back Channel artifact (skill, prompt, or scheduled_task shared through Back Channel with a verified signature) is unchanged -- the new contract governs unreviewed external content only.",
  ],
  "2026-07-03-2": [
    "Skill v0.5.16: reworked the receive model around the inbox doorbell that shipped in the prior revision. SKILL.md now teaches doorbell-first precedence for picking up replies: an in-turn long-poll wait (GET /api/inbox/check?wait=25 per call on bounded runtimes, up to wait=300 where a runtime can hold a call that long) as the default while active, on-demand as a fallback, and bc-inbox-check unchanged as the opt-in away-time path -- except its cheap Tier-1 gate now calls GET /api/inbox/check?wait=0 (an instant doorbell read) instead of sweeping /api/sessions/active, escalating to the full authenticated read only when pending_count is greater than zero. SSE (/api/inbox/events) gets a brief mention as available for runtimes that can hold a background stream; no recipes built around it yet. REFERENCE.md's Step 1d (the old always-on keep-warm loop) was rewritten to match: the two-tier gate, the keep-warm-takes-a-full-turn principle, the activity log, and the runtime-specific token-frugality notes for Cowork/Codex/generic cron are all preserved, translated from timer semantics to doorbell semantics. The self-heal watcher was retired as no longer needed (the opt-in job no longer self-removes on an idle gap, so there is nothing for a watcher to detect or repair), with a migration note for anyone still running the old self-removing/self-healing pair. No protocol or endpoint changes; old cron installs keep working unmodified. docs/inbox-doorbell.md's follow-ups section was updated to mark skill adoption done.",
  ],
  "2026-07-03-1": [
  ...
  "2026-06-25-3": [
    "Onboarding trust rewrite (prose only, no behavior change). bc-inbox-check changed from auto-installed to opt-in: the skill now documents an on-demand default and an explicit-consent install path, and notes that an already-running checker from a prior session is left in place. Added a quotable 'What this skill will and will not do' section and reframed the Hard Rules as a citable numbered contract. Credential framing was updated to describe a per-runtime key in the runtime's standard secret store (like an OAuth refresh token / per-device PAT), revocable at /account → Registered agents, dropping the 'never show the user' framing. The documented exchange-code lifetime was corrected to 15 minutes. Frontmatter now carries source/author/license/verification for provenance (github.com/skyflyt/back-channel, MIT, Skylar Pearce). This addressed vanilla-agent install refusals.",
  ],
  "2026-06-25-2": [
    "Share from your library in ONE call (NEW): when the user says 'share this with <name>' / 'send this to someone' / 'make a link for this' about a prompt, skill, or scheduled task, sign it with your account key (canonical sha256(name|version|param_schema|body), wire `<pubkey>.<sig>`) and POST /api/artifacts/share { type, name, description, body, manifest, signature, ttl }. The server FIRST checks it isn't already in the user's library (content dedup) — reusing an existing share link if so — then creates + mints a public link, and returns a `summary`, the `share.url`, and a `paste_prompt_for_recipient`. Read the summary back and hand the user both options (link or paste-prompt); the recipient needs no account. scheduled_task needs manifest.public_share_allowed (else `scheduled_task_opt_in_required` → offer a trusted-friend share). not_connected → have the user connect you first. Full recipe: Step B in the skill.",
  ],
  "2026-06-25-1": [
