...
3. **Privacy.**
   ...
   - The only history is the owner's **7-day connection log**: which device reached which PC, and
     when. It has no duration, bytes or address, and rows older than 7 days are deleted.
4. **Conventions.**
   - JSON bodies with exactly the specified members, at most 4 KiB.
   - `Cache-Control: no-store`, errors as `{ "error": "<code>" }`.
   - Rate limits keyed by device, account or the relay's identity, never by IP.
...
| `DELETE /account/devices/{id}` | — | `204`. Revokes the device's credentials and withdraws its pairings; a live relay session ends at its next lease renewal. |
| `GET /account/connections` | — | `{ connections: [{ hostDeviceId, remoteDeviceId, at }] }`, last 7 days |
...
| `POST /devices/self/credential` | `appbridge.device` | — | `{ credential, expiresAt }`. The old credential stops working. |
...
| `PUT /hosts/self/pairings/{enrollmentId}` | `appbridge.host.relay` | `{ remoteDeviceId }` | `204`. Idempotent; a withdrawn enrollment is never revived (`409`). |
| `DELETE /hosts/self/pairings/{enrollmentId}` | `appbridge.host.relay` | — | `204` |
